...
for M4A in ./*m4a; do
  ...
  #$DRY alac "$M4A" | sox - -c 2 -b 16 "$(echo $M4A | sed -e s/.m4a/_both.wav/g)" remix 1-2;
  # mix down to MONO file and convert to vbr mp3
  # sed version
  #$DRY alac "$M4A" | sox - -b 16 -t wav - remix 1-2 | lame -q 0 -vbr-new -V 0 - "$(echo $M4A | sed -e s/.m4a/_both.mp3/g)"
