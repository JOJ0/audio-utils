...
for MP3 in ./*mp3; do
  ...
  #$DRY sox "$MP3" -t raw -e signed-integer -c 1 -b 16 -r 44100 - -c 1 -b 16 "$(echo $MP3 | sed -e s/.mp3/_mono.wav/g)";
  #
  ...
  #$DRY sox "$MP3" -c 2 -b 16 "$(echo $MP3 | sed -e s/.mp3/_both.wav/g)" remix 1-2;
  # mix down to MONO file and convert to vbr mp3
  # sed version
  #$DRY sox "$MP3" -b 16 -t wav - remix 1-2 | lame -q 0 -vbr-new -V 0 - "$(echo $MP3 | sed -e s/.mp3/_both.mp3/g)"
  ...
  $DRY sox "$MP3" -b 16 -t wav - remix 1-2 | $DRY lame -q 0 -vbr-new -V 0 --id3v2-only --tn "$TRACKNO" --ta "$ARTIST" --ty "$YEAR" --tl "$ALBUM" --tt "$TITLE BOTH" - "$FILENAME" 
done
