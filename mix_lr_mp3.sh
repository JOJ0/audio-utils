...
for MP3 in ./*mp3; do
  ...
  #$DRY alac "$MP3" | sox - -c 2 -b 16 "$(echo $MP3 | sed -e s/.mp3/_both.wav/g)" remix 1-2;
  # mix down to MONO file and convert to vbr mp3
  # sed version
  #$DRY alac "$MP3" | sox - -b 16 -t wav - remix 1-2 | lame -q 0 -vbr-new -V 0 - "$(echo $MP3 | sed -e s/.mp3/_both.mp3/g)"
